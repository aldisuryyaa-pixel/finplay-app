...
import { LayoutDashboard, Wallet, TrendingUp, TrendingDown, Clock, Trash2, LogOut, Command, PieChart as ChartIcon, Eye, EyeOff, Plus, X, Filter, Target, CalendarDays, Settings, HelpCircle, User, Sparkles, ShieldCheck, Menu as MenuIcon, Send, Mail, Moon, Sun, FileText, Fingerprint, Download, Calculator, BarChart3, Activity, ShieldAlert, Flame, Camera, Utensils, Car, Zap, Film, ShoppingBag, ArrowUpRight, AlertTriangle, LifeBuoy, Users, Award, Coins, Scale, CheckCircle2 } from "lucide-react";
...
export default function Home() {
  ...
  // v3.5 BARU: Algoritma Penghitung Kemajuan Angka Payout Pangkat Finansial (Rank Progress Gauge)
  ...
  // v3.5 BARU: Kompilation Koleksi 5 Grid Misi Finansial Adaptif Waktu Nyata
  ...
  // v3.5 BARU: Penghitung Persentase Komposisi Alokasi Diversifikasi Kekayaan Bersih
        ...
        <header className="h-20 lg:h-24 flex items-center justify-between px-4 sm:px-6 lg:px-10 bg-white/70 dark:bg-[#0B0F19]/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 sticky top-0 z-30 transition-colors">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            {activeMenu !== "Asisten AI" && activeMenu !== "Pengaturan Akun" && activeMenu !== "Perencana Finansial" && activeMenu !== "Pusat Pengaduan" && activeMenu !== "Portofolio Aset" && activeMenu !== "Misi Finansial" && activeMenu !== "Split & Shared" && (
              <button onClick={() => setModalConfig({ isOpen: true, type: activeMenu === "Target Finansial" ? "goal" : activeMenu === "Tagihan Berkala" ? "bill" : "trx" })} className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-emerald-500 text-slate-900 rounded-2xl font-bold text-xs lg:text-sm shadow-xl shadow-emerald-500/20 hover:scale-105 transition-all"><Plus size={16} /> <span>Entri Modul</span></button>
            )}
          </div>
        </header>
          ...
          {activeMenu === "Portofolio Aset" && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 max-w-4xl mx-auto">
              <div className="p-6 bg-slate-900 rounded-3xl border border-slate-800 text-center">
                ...
                {/* v3.5 UPDATE: Komponen Bilah Keseimbangan Alokasi Komposisi Aset Makro */}
                <div className="mt-6 space-y-2">
                  <div className="flex h-3 rounded-full overflow-hidden bg-slate-800">
                    <div style={{ width: `${assetDistributionPercentages.kas}%` }} className="bg-emerald-400 transition-all duration-500" title="Kas Dompet" />
                    <div style={{ width: `${assetDistributionPercentages.saham}%` }} className="bg-sky-400 transition-all duration-500" title="Saham" />
                    <div style={{ width: `${assetDistributionPercentages.emas}%` }} className="bg-amber-400 transition-all duration-500" title="Emas" />
                    <div style={{ width: `${assetDistributionPercentages.reksadana}%` }} className="bg-purple-400 transition-all duration-500" title="Reksadana" />
                  </div>
                  <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-[10px] font-black uppercase text-slate-400">
                    ...
                    <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded bg-sky-400 block" /> Saham ({assetDistributionPercentages.saham.toFixed(0)}%)</div>
          ...
          {activeMenu === "Misi Finansial" && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 max-w-2xl mx-auto">
              <div className={`p-6 rounded-3xl border ${financialHealth.bg} ${financialHealth.color}`}>
                ...
                {/* v3.5 UPDATE: Bilah Visual Komputasi Meteran Kenaikan Pangkat Finansial */}
              ...
              <div className="bg-white dark:bg-[#0F172A] p-6 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-4">
                ...
                {/* v3.5 UPDATE: Peremajaan Grid 5 Misi Finansial Otomatis */}
